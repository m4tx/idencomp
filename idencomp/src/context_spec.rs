...
/// The generic context specifier, that stores full data about last `ACID_ORDER`
/// acids, last `Q_SCORE_ORDER` quality scores and the position in a sequence
/// using `POSITION_BITS` bits.
...
{
    /// Creates a new `GenericContextSpec`.
    ///
    /// # Examples
    /// ```
    ...
    ///
    /// let _spec =
    ///     GenericContextSpec::<2, 1, 4>::new([Acid::A, Acid::G], [FastqQualityScore::new(5)], 3);
    /// ```
    ///
    /// # Panics
    /// Panics if `position` argument is grater than `1 << POSITION_BITS`.
    #[must_use]
    pub const fn new(
        acids: [Acid; ACID_ORDER],
        q_scores: [FastqQualityScore; Q_SCORE_ORDER],
...
{
    /// Creates a new `GenericContextSpec` with `position` set to 0. Implies
    /// `POSITION_BITS == 0`.
    ///
    /// # Examples
    /// ```
    ...
    /// use idencomp::sequence::Acid;
    ///
    /// let _spec = GenericContextSpec::without_pos([Acid::A, Acid::G], [FastqQualityScore::new(5)]);
    /// ```
    #[must_use]
...
/// An object that is capable of going through the symbols one by one and
/// generate [`ContextSpec`] instances at each step. The exact method of
/// generating the [`ContextSpec`]s is chosen by the implementations of this
/// trait.
pub trait ContextSpecGenerator {
    /// Gets the current context as a [`ContextSpec`] instance.
    ///
    /// Please note that this should also work properly if [`Self::update()`]
    /// has never been called before.
    ///
    /// # Examples
    /// ```
    /// use idencomp::context_spec::{ContextSpecGenerator, GenericContextSpecGenerator};
    ///
    /// let mut generator = GenericContextSpecGenerator::<2, 2, 3>::new(10);
    /// assert_eq!(generator.current_context().get(), 0);
    /// ```
    #[must_use]
    fn current_context(&self) -> ContextSpec;

    /// Move the current context by one symbol, with given [`Acid`] and
    /// [`FastqQualityScore`].
    ///
    /// # Examples
    /// ```
...
{
    /// Creates a new `GenericContextSpecGenerator` instance.
    ///
    /// # Examples
    /// ```
    /// use idencomp::context_spec::{ContextSpecGenerator, GenericContextSpecGenerator};
    ...
    /// # Examples
    /// ```
    /// use idencomp::context_spec::GenericContextSpecGenerator;
    ///
    /// assert_eq!(GenericContextSpecGenerator::<2, 1, 5>::spec_num(), 131072);
    /// ```
    #[must_use]
    ...
    > LightContextSpecGenerator<ACID_ORDER, Q_SCORE_ORDER, POSITION_BITS, Q_SCORE_MAX>
{
    ...
    /// # Examples
    /// ```
    /// use idencomp::context_spec::{ContextSpecGenerator, LightContextSpecGenerator};
    /// use idencomp::fastq::FastqQualityScore;
    /// use idencomp::sequence::Acid;
    ///
    /// let mut generator = LightContextSpecGenerator::<2, 2, 5, 16>::new(10);
    /// assert_eq!(generator.current_context().get(), 0);
    /// generator.update(Acid::G, FastqQualityScore::new(5));
    /// assert_ne!(generator.current_context().get(), 0);
    /// ```
    ...
    /// # Examples
    /// ```
    /// use idencomp::context_spec::LightContextSpecGenerator;
    ///
    /// assert_eq!(LightContextSpecGenerator::<2, 1, 5, 16>::spec_num(), 8192);
    /// ```
    #[must_use]
    pub const fn spec_num() -> u32 {
        1 << Self::total_bits()
